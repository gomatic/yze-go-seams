...
// support, so the clock-bound branch below is still reported.
//
...
// Expired branches on the real clock from production code, which is reported:
// the package is in scope despite having a test that imports `testing`.
